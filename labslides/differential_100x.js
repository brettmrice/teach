...
(function () {
  ...
  function createStartupTaskModal() {
    ...
    if (diffBtn) {
      diffBtn.onclick = (e) => {
        e.preventDefault();
        closeTaskModal();
        // Expand Diff panel, collapse Platelet panel
    ...
    if (pltBtn) {
      pltBtn.onclick = (e) => {
        e.preventDefault();
        closeTaskModal();
        // Collapse Diff panel, expand Platelet panel
        ...
        // Show platelet procedure if not suppressed
        if (window.__Counter100X && typeof window.__Counter100X.showProcedure === "function") {
          try {
            if (localStorage.getItem("counter100x_hide_procedure") !== "1") {
              window.__Counter100X.showProcedure();
            }
          } catch (err) {
            window.__Counter100X.showProcedure();
          }
  ...
  function initDifferential() {
    ...
    function updateTaskPositions() {
      ...
      const gap = 16; // clean 16px gap between active task and collapsed task badge
      ...
      if (diffExpanded && counterWrapper && counterWrapper.classList.contains("minimized")) {
        ...
        const collapsedBadgeHeight = counterCard ? counterCard.offsetHeight : 54;
        const offsetPx = Math.round(diffHeight / 2 + gap + collapsedBadgeHeight / 2);
        ...
        counterWrapper.classList.remove("displaced-top");
      } else if (counterExpanded && diffWrapper && diffWrapper.classList.contains("minimized")) {
        const counterCard = counterWrapper.querySelector(".counter100x-card");
        const diffCard = diffWrapper.querySelector(".diff100x-card");
        const counterHeight = counterCard ? counterCard.offsetHeight : 450;
        const collapsedBadgeHeight = diffCard ? diffCard.offsetHeight : 54;
        const offsetPx = Math.round(counterHeight / 2 + gap + collapsedBadgeHeight / 2);
        diffWrapper.style.setProperty("--diff100x-displaced-top", `calc(-50% - ${offsetPx}px)`);
        diffWrapper.classList.add("displaced-top");
        diffWrapper.classList.remove("displaced-bottom");
      } else {
        if (diffWrapper) {
          diffWrapper.classList.remove("displaced-top", "displaced-bottom");
          diffWrapper.style.removeProperty("--diff100x-displaced-top");
          diffWrapper.style.removeProperty("--diff100x-displaced-bottom");
        }
        if (counterWrapper) {
          counterWrapper.classList.remove("displaced-top", "displaced-bottom");
          counterWrapper.style.removeProperty("--counter100x-displaced-top");
          counterWrapper.style.removeProperty("--counter100x-displaced-bottom");
        }
      }
    }
    ...
    window.__Differential100X = {
      expand: () => {
        // If Counter100X is currently expanded, collapse it first to prevent overlapping transitions
